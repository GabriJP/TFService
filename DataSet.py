...
class DataSet:
    ...
    def from_videos(cls, videos, new_dimensions, crop_dimensions, train_pct, test_pct):
        pool = ThreadPool()
        results = [pool.apply_async(cls.get_frames, (video, new_dimensions, crop_dimensions)) for label, video in videos]
        pool.close()
        pool.join()
    ...
    @staticmethod
    def get_frames(filename, new_dimensions, crop_dimensions):
